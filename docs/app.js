...
async function init() {
  ...
  const PALETTE = [
    ...
    // BKK official line colors (uppercase — exactly as the pipeline emits
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      a.download = `budapest-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `budapest-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the H6 HÉV runs 40 km south to Ráckeve, so
  // fitting the data bbox would open on countryside with the city as a small
  // core. Open on central Budapest instead.
  map.jumpTo({ center: [19.055, 47.497], zoom: 11.4 });
}
