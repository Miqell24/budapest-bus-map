...
// and HÉV keep their official colors straight from routes.txt and trigger the
// engine's metro treatment (wide ribbon, station discs, always-on names) via
// the M/H-prefixed line keys. Replacement services (route_id VP*/TP*/MP* —
// villamos-/troli-/metrópótló buses that duplicate rail numbers) are skipped.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|4,M2,H5]
...
// M1–M4 and H5–H9 both get the metro treatment; trams are everything else
const isRailTrunk = (l) => /^[MH]\d/.test(l);
const tramSel = tramLines.filter((l) => !isRailTrunk(l));
const metroSel = tramLines.filter((l) => /^M\d/.test(l));
const hevSel = tramLines.filter((l) => /^H\d/.test(l));
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/budapest-rail.json',
  // the Fogaskerekű (line 60, Széchenyi-hegy rack railway) is BKK tram route 60
  // but OSM tags it railway=light_rail + rack — without it the line collapsed
  // to 0.2 km of garbage (175 observations with no candidate track)
  graphMode: 'tram', railKeep: new Set(['tram']),
  railExtra: (e) => e.tags?.railway === 'light_rail' && e.tags?.rack,
...
if (tramAll || metroSel.length) MODES.push({
  mode: 'tram', label: 'metro', osmFile: 'data/osm/budapest-rail.json',
...
if (tramAll || hevSel.length) MODES.push({
  // MÁV-HÉV suburban lines: own agency in the feed (109), own official colors,
  // running on light_rail/rail track — a separate slice of the rail graph so
  // Viterbi cannot stray onto tram or subway rails
  mode: 'tram', label: 'HÉV', osmFile: 'data/osm/budapest-rail.json',
  graphMode: 'tram', railKeep: new Set(['light_rail', 'rail']),
...
await (await import('./night.mjs')).nightPass(outDir, /^9\d\d$/, { sort: false });
